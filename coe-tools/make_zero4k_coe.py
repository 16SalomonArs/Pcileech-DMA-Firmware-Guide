    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as source:
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            if offset >= 4096 or offset % 4:
                raise ValueError(f"Zero4K offset must be DWORD-aligned from 0x000 to 0xFFC: 0x{offset:X}")
            if value > 0xFFFFFFFF:
...
def main(arguments=None):
    ...
    if len(arguments) != 2:
        raise SystemExit("Usage: python make_zero4k_coe.py bar_dwords.csv pcileech_bar_zero4k.coe")
    try:
        dwords = build_dwords(Path(arguments[0]))
    except ValueError as error:
        raise SystemExit(str(error)) from error
    output_path = Path(arguments[1])
    write_coe(output_path, dwords)
    print(f"Wrote {output_path}")
    print(f"0x000: {dwords[0]}")
    print(f"0xFFC: {dwords[-1]}")
